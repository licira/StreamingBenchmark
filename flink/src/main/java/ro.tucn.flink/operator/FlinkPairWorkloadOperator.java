...
public class FlinkPairWorkloadOperator<K, V> extends PairWorkloadOperator<K, V> {
    ...
    public FlinkPairWorkloadOperator(DataStream<Tuple2<K, V>> dataStream, int parallelism) {
        super(parallelism);
        this.dataStream = dataStream;
    }

    /*public FlinkGroupedWorkloadOperator<K, V> groupByKey() {
        KeyedStream<Tuple2<K, V>, Object> keyedStream = dataStream.keyBy(new KeySelector<Tuple2<K, V>, Object>() {
            @Override
     ...
     */
    @Override
    public PairWorkloadOperator<K, V> reduceByKeyAndWindow(final ReduceFunction<V> fun, String componentId, TimeDuration windowDuration, TimeDuration slideDuration) {
