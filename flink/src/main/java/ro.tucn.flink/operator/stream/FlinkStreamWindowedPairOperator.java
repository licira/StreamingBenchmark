 ...
 */
public class FlinkStreamWindowedPairOperator<K, V, W extends Window> extends StreamWindowedPairOperator<K, V> {

    private WindowedStream<Tuple2<K, V>, K, W> windowStream;

    public FlinkStreamWindowedPairOperator(WindowedStream<Tuple2<K, V>, K, W> stream, int parallelism) {
        super(parallelism);
        windowStream = stream;
    }
    ...
    @Override
    public <R> StreamPairOperator<K, R> mapPartition(final MapPartitionFunction<Tuple2<K, V>, Tuple2<K, R>> fun, String componentId) {
    ...
    @Override
    public <R> StreamPairOperator<K, R> mapValue(final MapFunction<Tuple2<K, V>, Tuple2<K, R>> fun, String componentId) {
        DataStream<Tuple2<K, R>> newDataStream = windowStream.apply((WindowFunction<Tuple2<K, V>, Tuple2<K, R>, K, W>) (k, window, values, collector) -> {
    ...
    @Override
    public StreamPairOperator<K, V> filter(final FilterFunction<Tuple2<K, V>> fun, String componentId) {
    ...
    @Override
    public StreamPairOperator<K, V> reduce(final ReduceFunction<Tuple2<K, V>> fun, String componentId) {
        DataStream<Tuple2<K, V>> newDataStream = windowStream.reduce((org.apache.flink.api.common.functions.ReduceFunction<Tuple2<K, V>>) (t1, t2) ->
                fun.reduce(t1, t2));
        return new FlinkStreamPairOperator<>(newDataStream, parallelism);
    }
